...
use Opensoft\Bundle\GversationBundle\Git\Builder;
...
class SynchronizeCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('opensoft:gversation:sync');
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($projectName = $input->getArgument('name')) {
            $projects = array($em->getRepository('OpensoftGversationBundle:Project')->findOneBy(array('name' => $projectName)));
        } else {
            $projects = $em->getRepository('OpensoftGversationBundle:Project')->findAll();
        }
